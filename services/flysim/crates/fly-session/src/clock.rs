...
//!
//! The accumulator is exact rational arithmetic, never rounded nanoseconds. A 60 Hz world with
//! a 1 ms model tick advances 16, 17, 17 ticks over its first three steps and comes back to a
//! remainder of exactly zero; rounding to microseconds does not.

use crate::types::{DomainError, ErrorCode, RationalNs, U64};
...
impl TickAccumulator {
    ...
    pub fn new(tick_duration: RationalNs) -> Result<TickAccumulator, String> {
        tick_duration.validate()?;
        if !tick_duration.is_positive() {
            return Err("tick duration must be positive".to_owned());
        }
        Ok(TickAccumulator {
            tick_duration,
    ...
    pub fn advance(&mut self, interval: &RationalNs) -> Result<u64, String> {
        ...
        if !interval.is_positive() {
            return Err("environment interval must be positive".to_owned());
        }
        let accumulated = self.remainder.checked_add(interval)?;
        let ticks = accumulated.checked_div_floor(&self.tick_duration)?;
        let consumed = self.tick_duration.checked_mul_u64(ticks)?;
        let remainder = accumulated.checked_sub(&consumed)?;
        debug_assert_eq!(
            remainder.cmp_value(&self.tick_duration),
...
impl Pacing {
    ...
    fn period(&self) -> std::time::Duration {
        let ns = u128::from(self.step_duration.numerator.0) / u128::from(self.step_duration.denominator.0).max(1);
...
pub fn ticks_to_legacy_millis(ticks: u64, tick_duration: &RationalNs) -> Result<f64, DomainError> {
    ...
    let per_tick_ms = tick_duration.numerator.0 as f64
        / (tick_duration.denominator.0 as f64 * 1_000_000.0);
...
mod tests {
    ...
    fn a_60_hz_world_with_a_1_ms_tick_runs_16_17_17() {
        let step = RationalNs::from_hz(60).unwrap();
        let mut acc = TickAccumulator::new(RationalNs::from_millis(1).unwrap()).unwrap();
    ...
    fn the_remainder_stays_below_one_tick_and_never_goes_negative() {
        let step = RationalNs::from_hz(60).unwrap();
        let tick = RationalNs::from_millis(1).unwrap();
        let mut acc = TickAccumulator::new(tick).unwrap();
        for _ in 0..600 {
            acc.advance(&step).unwrap();
            assert_eq!(acc.remainder().cmp_value(&tick), std::cmp::Ordering::Less);
        }
    ...
    fn warm_up_ticks_do_not_touch_the_remainder() {
        ...
        assert!(acc.remainder().is_zero());
        assert_eq!(acc.advance(&RationalNs::from_hz(60).unwrap()).unwrap(), 16);
    }
